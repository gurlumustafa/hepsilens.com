...
export default function FilterSidebar({ filters, onChange, mode = "lens", mobileOpen = false, onMobileClose }: Props) {
  ...
  const filterHeader = (withClose = false) => (
    <div
      ...
      style={{ background: "linear-gradient(135deg, #f0f4ff 0%, #fefce8 100%)", borderBottom: "1.5px solid #e2e4ee" }}
    >
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "#dae2ff" }}>
          <span className="material-symbols-outlined" style={{ fontSize: "17px", color: "#003d9b", fontVariationSettings: "'FILL' 1" }}>tune</span>
        </div>
        <div>
          <p style={{ fontSize: "15px", fontWeight: 800, color: "#191c1e", fontFamily: "'Plus Jakarta Sans'", letterSpacing: "-0.01em" }}>Filtrele</p>
          <p style={{ fontSize: "11px", color: "#737685", fontFamily: "'Inter'" }}>Gözlerinize özel hassas uyum</p>
        </div>
        {activeCount > 0 && (
          <span className="px-2.5 py-1 rounded-full" style={{ fontSize: "10px", fontWeight: 800, background: "#003d9b", color: "#fff", letterSpacing: "0.05em" }}>
            {activeCount}
          </span>
        )}
      </div>
      {withClose && (
        <button onClick={onMobileClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#e2e4ee] transition-colors">
    ...
    <div className="flex flex-col divide-y divide-[#f0f1f5]">
      <div className="px-4 py-5">
        ...
        <div className="flex flex-col gap-1">
          {brands.map((b) => (
            <CheckRow key={b.id} checked={filters.brands.includes(b.id)} onChange={() => toggleBrand(b.id)} label={b.name} />
          ))}
        </div>
      </div>
      {mode === "lens" && (
        <div className="px-4 py-5">
          ...
          <div className="flex flex-col gap-1">
            {DURATIONS.map((d) => (
              <CheckRow key={d.value} checked={filters.usage.includes(d.value)} onChange={() => toggleUsage(d.value)} label={d.label} icon={d.icon} />
            ))}
          </div>
        </div>
      )}
      {mode === "accessories" && (
        <div className="px-4 py-5">
          ...
          <div className="flex flex-col gap-1">
            {CATEGORIES.map((c) => (
              <CheckRow key={c.value} checked={filters.category.includes(c.value)} onChange={() => toggleCategory(c.value)} label={c.label} icon={c.icon} />
            ))}
          </div>
        </div>
      )}
  ...
  const clearButton = (
    <div className="px-4 py-4" style={{ borderTop: "1.5px solid #e2e4ee", background: "#fafbff" }}>
      <button
        ...
        style={{
          fontSize: "12.5px", letterSpacing: "0.04em", fontFamily: "'Inter'",
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ background: "#fff", border: "1.5px solid #e2e4ee", borderRadius: "18px", boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}
      >
        {filterHeader(false)}
        {filterGroups}
        {clearButton}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[60] lg:hidden flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-2xl overflow-hidden flex flex-col" style={{ maxHeight: "85vh" }}>
            {filterHeader(true)}
            <div className="overflow-y-auto flex-1">{filterGroups}</div>
            {clearButton}
          </div>
        </div>
      )}
